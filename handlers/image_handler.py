...
import png  # in order to use png run pip install pypng
...
class ImageHandler:
    ...
    def save_image(self, img_name):
        data_to_save = np.floor(self.image * (2 ** 16 - 1))
        data_to_save = data_to_save.astype('uint16')
        png.from_array(data_to_save, mode='L').save(fh.get_output_img_path(img_name))
